...
# Check if the pythonPath parameter is provided
if [ -z "$1" ]; then
  if [[ "$(uname)" == "Linux" ]]; then
...
echo "------Defining Python environment for $pythonPath ------"
$pythonCmd -m pip install --root-user-action=ignore numpy
...
pythonLibName="libpython$($pythonCmd --version | awk '{print $2}' | cut -d '.' -f 1,2)${libSuffix}"

pythonVersion=$($pythonCmd --version | awk '{print $2}')
...
export Python3_NumPy_INCLUDE_DIRS=$($pythonCmd -c "import numpy; print(numpy.get_include())")
...
echo "------Finished defining Python environment for $pythonPath ------"